...
class TestHelpers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.file_package = open(
            join(self.package_folder, self.file_package_name), "w+")
        ...
        self.inner_file = open(
            join(join(self.package_folder, self.inner_folder), self.inner_file_name), "w+")
        self.inner_init = open(
            join(join(self.package_folder, self.inner_folder), self.init_name), "w+")
    ...
    def test_ispackage_path_not_package(self):
        self.assertFalse(
            helpers.ispackage_path(abspath(self.not_package_folder_name)))
    ...
    def test_safe_import_import(self):
        self.assertIsNotNone(
            helpers.safe_import("package_folder.file_package"))
    ...
    def test_extract_attributes_not_right(self):
        ...
        import testfile
        self.assertNotEqual(
            [], helpers.extract_attributes(self.file_current_folder))
    ...
    def test_extract_submodules_successfully(self):
        ...
        module_tree = [package_folder, package_folder.file_package,
                       package_folder.inner_folder, package_folder.inner_folder.inner_file]
